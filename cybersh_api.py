...
def _resolve_user(
    ...
    Resolve the effective user_id from:
      1. JWT via Authorization: Bearer (most secure, verifies with Supabase)
      2. X-User-Id header (trusted only if auth is not configured — dev mode)
      3. Falls back to 'anon'

    In production both JWT and the derived user_id should match.
    """
...
# ─────────────────────────────────────────────
#  MEMORY HELPERS  (per-user, ISOLATED)
# ─────────────────────────────────────────────
...
def load_session(filename: str, user_id: str = Depends(_uid)):
    ...
    try:
        ...
        raise HTTPException(500, "Could not parse session file.")
...
def search_sessions(keyword: str, user_id: str = Depends(_uid)):
    ...
    for fpath in files:
        try:
            ...
            get_safe_session_path(basename, user_id)
            with open(fpath, encoding="utf-8") as f:
                data = json.load(f)
            hits = []
            ...
            if hits:
                found.append({
                    ...
                    "filename": basename, "hits": hits[:3], "total_hits": len(hits),
                })
        except Exception:
            pass
