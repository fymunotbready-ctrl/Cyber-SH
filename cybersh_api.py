...
╚██████╗   ██║   ██████╔╝███████╗██║  ██║    ███████║██║  ██║
 ╚═════╝   ╚═╝   ╚═════╝ ╚══════╝╚═╝  ╚═╝    ╚══════╝╚═╝  ╚═╝
  CYBER SH — FastAPI Web Backend  (v1.5.1)

  v1.5.1 Bug Fixes:
  - FIX CRITICAL: CORS allow_origins=["*"] + allow_credentials=True is rejected by
    browsers. Now uses ALLOWED_ORIGINS env var (defaults to localhost for dev).
  - FIX CRITICAL: _MEMORY_EMPTY.copy() was a shallow copy — nested lists/dicts were
    SHARED across all users causing memory bleed. Fixed with per-call factory.
  - FIX: _resolve_user._last_error used a function attribute (not thread-safe under
    concurrent requests). Replaced with threading.local().
  - FIX: Missing /api/ai/git-log endpoint — AIGitLogRequest model existed with no route.
  - FIX: _auto_learn() called synchronously inside streaming generator — now runs in a
    daemon thread so it never blocks stream delivery to the client.
  - FIX: Removed unused imports (math, textwrap, subprocess, sys).
  - FIX: Moved StaticFiles import to top with other imports.
  - IMPROVED: _sb_get / _sb_set now log errors instead of silently swallowing them.
  - IMPROVED: Streaming error handler always yields [DONE] so client doesn't hang.
...
logger = logging.getLogger("cybersh.api")
...
#  Each user gets ~/.cybersh/<user_id>/ for local (anon) storage.
#  Authenticated users get Supabase-backed storage instead.
#  Anonymous sessions use "anon" bucket — fine for single-device,
#  but multi-user MUST authenticate to isolate data properly.
# ─────────────────────────────────────────────
...
# FIX: was using a function attribute (_resolve_user._last_error) which is not
# thread-safe under concurrent requests. Replaced with threading.local().
_tl = threading.local()
...
def _resolve_user(
    ...
    """
    _tl.last_error = ""  # reset error state for this request thread
...
#  )
#
#  Anonymous users fall back to local files.
# ─────────────────────────────────────────────
...
def _sb_get(user_id: str, field: str, default):
    ...
    try:
        ...
        logger.warning("_sb_get(%s, %s) failed: %s", user_id[:8], field, exc)
...
def _sb_set(user_id: str, field: str, value) -> None:
    ...
    try:
        ...
        logger.warning("_sb_set(%s, %s) failed: %s", user_id[:8], field, exc)
...
def _empty_memory() -> dict[str, Any]:
    """
    FIX: was using _MEMORY_EMPTY.copy() which is a shallow copy — the nested
    lists and dicts (facts, preferences, projects, learned) were SHARED across
    all users hitting the default path. This factory creates a fully independent
    fresh object every time.
    """
    return {"facts": [], "preferences": {}, "projects": {}, "learned": []}
...
def load_memory(user_id: str = "anon") -> dict:
    ...
    return _sb_get(user_id, "memory", _empty_memory)
...
def memory_context(mem: dict) -> str:
    if not any(mem.get(k) for k in ("facts", "preferences", "projects", "learned")):
        return ""
...
#  Extracts learnable facts from a conversation turn and stores them
#  in the user's memory — making each user's AI progressively more
#  personalised. Runs in a background thread so it never blocks responses.
# ─────────────────────────────────────────────
...
def _auto_learn(user_msg: str, ai_reply: str, user_id: str) -> None:
    """
    Extract learnable facts from a conversation turn.
    FIX: now called in a daemon thread from both streaming and non-streaming
    paths so it never blocks response delivery to the client.
    """
    try:
        ...
        logger.debug("_auto_learn failed (non-critical): %s", exc)


def _auto_learn_bg(user_msg: str, ai_reply: str, user_id: str) -> None:
    """Fire-and-forget wrapper: runs _auto_learn in a daemon thread."""
    t = threading.Thread(target=_auto_learn, args=(user_msg, ai_reply, user_id), daemon=True)
    t.start()
...
def whoami(
    ...
    # FIX: was reading from a non-thread-safe function attribute; now uses threading.local
    auth_error = getattr(_tl, "last_error", "") or None
    ...
    return {
        ...
        "hint": (
            ...
            if user_id != "anon"
                ...
                "No auth header sent — frontend must include: Authorization: Bearer <access_token>"
    ...
    if req.stream:
        ...
        def event_stream():
            try:
                ...
                # FIX: _auto_learn now fires in a daemon thread — doesn't block stream close
                if req.save_history and last_user_msg:
                    append_to_history(last_user_msg, ai_reply, req.mode, user_id)
                    _auto_learn_bg(last_user_msg, ai_reply, user_id)

            except Exception as exc:
                logger.error("Streaming error for user %s: %s", user_id[:8], exc)
                yield f"data: {json.dumps({'error': 'Streaming error.'})}\n\n"
            finally:
                # FIX: [DONE] is always yielded so the client never hangs
                yield "data: [DONE]\n\n"
        ...
        if req.save_history and last_user_msg:
            ...
            _auto_learn_bg(last_user_msg, response, user_id)
...
def delete_memory(req: MemoryDeleteRequest, user_id: str = Depends(_uid)):
    ...
    if keyword in mem.get("projects", {}):
        del mem["projects"][keyword]
        removed += 1
    if keyword in mem.get("preferences", {}):
        del mem["preferences"][keyword]
        removed += 1
...
def clear_memory(user_id: str = Depends(_uid)):
    save_memory(_empty_memory(), user_id)
    return {"ok": True}
...
@app.post("/api/ai/git-log")
def ai_git_log(req: AIGitLogRequest, user_id: str = Depends(_uid)):
    """
    FIX: AIGitLogRequest model was defined but this endpoint was never registered.
    Helps users understand, query, and analyse git log output with AI.
    """
    parts = [f"Help me with git log"]
    if req.target:
        parts.append(f"for target/ref: {req.target}")
    if req.repo_path:
        parts.append(f"in repo: {req.repo_path}")
    parts.append(
        f"Show how to display the last {req.limit} commits, explain the output format, "
        f"and give useful git log flags for filtering, formatting, and graphing history."
    )
    result = ai_complete(
        " ".join(parts),
        mode=req.mode or "code", persona=req.persona, user_id=user_id,
    )
    return {"result": result}
...
def tool_passgen(type: str = Query("password"), length: int = Query(16)):
    length = max(8, min(length, 128))
    if type == "phrase":
        _words = [
            "correct", "battery", "horse", "staple", "crimson", "falcon", "thunder",
            "whisper", "golden", "silver", "crystal", "shadow", "purple", "winter",
            "summer", "autumn", "spring", "bright", "swift", "brave", "noble", "cosmic",
            "lunar", "solar", "arctic", "delta", "echo", "ghost", "hunter", "cipher",
        ]
        result = "-".join(secrets.choice(_words) for _ in range(4))
    elif type == "token":
        result = secrets.token_hex(length // 2)
    else:
        alphabet = string.ascii_letters + string.digits + "!@#$%^&*"
        result = "".join(secrets.choice(alphabet) for _ in range(length))
    return {"type": type, "result": result, "length": len(result)}
...
def api_info():
    return {
        ...
        "docs":    "/docs",
        "new_in_1_5_1": [
            "CORS fix — ALLOWED_ORIGINS env var replaces wildcard+credentials bug",
            "Memory isolation fix — _empty_memory() factory prevents user memory bleed",
            "Thread-safety fix — _resolve_user uses threading.local instead of function attribute",
            "Missing /api/ai/git-log endpoint now registered",
            "_auto_learn runs in daemon thread — no longer blocks stream delivery",
            "sign_out / reset_password_for_email supabase-py v2 API fixes (in auth.py)",
        ],
        "new_in_1_5": [
            "User data isolation — all data scoped per authenticated user",
            "30+ language support via /api/languages",
