...
import { MODES, glassModal } from "./data"
...
export function ModeTag({ mode }: { mode: string }) {
  ...
  return (
    ...
    >
      <m.Icon s={10} c={m.color} /> {m.label.toUpperCase()}
    </span>
  )
}
...
export function ProfileMenu({
  ...
  const items = [
    ...
    { Icon: Ic.upgrade, label: "Upgrade", action: () => { onClose(); onOpenPanel("upgrade") } },
    null,
          ...
          item === null ? (
            ...
            <button
              ...
              style={{
                ...
                textAlign: "left",
                color: "danger" in item && item.danger ? "#ff453a" : "rgba(var(--text-rgb),0.85)",
              }}
              onMouseEnter={(e) => (e.currentTarget.style.opacity = "0.75")}
              onMouseLeave={(e) => (e.currentTarget.style.opacity = "1")}
            >
              <item.Icon s={17} c={"danger" in item && item.danger ? "#ff453a" : "rgba(var(--text-rgb),0.7)"} />
              <span style={{ fontSize: 14, fontWeight: 400, flex: 1 }}>{item.label}</span>
              {"hasArrow" in item && item.hasArrow && <Ic.chevR s={13} c="rgba(var(--tint-rgb),0.3)" />}
            </button>
          ),
        )}
      </div>
